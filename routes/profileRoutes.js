...
// ===== SKILL MANAGEMENT ROUTES (must come first) =====

// get all available skills: GET /api/profiles/skills/all
...
// add new skill: POST /api/profiles/skills/create
...
// delete skill: DELETE /api/profiles/skills/delete/:skillId
router.delete('/skills/delete/:skillId', async (req, res) => {
...
// search by skill: GET /api/profiles/search/by-skill
...
// add/update skills for profile: PUT /api/profiles/:profileId/skills
router.put('/:profileId/skills', async (req, res) => {
  ...
  try {
    ...
    const profileCheck = await client.query('SELECT id FROM profiles WHERE id = $1', [profileId]);
    ...
    const completeProfile = await getProfileWithSkills(profileId);
    const profile = ProfileFactory.createProfile(
...
// delete profile: DELETE /api/profiles/:profileId
router.delete('/:profileId', async (req, res) => {
  const { profileId } = req.params;
  try {
    // The CASCADE will automatically delete related profile_skills records
    const result = await pool.query('DELETE FROM profiles WHERE id = $1 RETURNING *', [profileId]);
